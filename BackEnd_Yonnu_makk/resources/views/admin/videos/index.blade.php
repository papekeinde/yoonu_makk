...
@section('content')
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="{{ route('admin.videos.create') }}" class="rounded-xl bg-yoonu-500 hover:bg-yoonu-600 text-white px-5 py-2.5 text-sm font-medium transition inline-flex items-center gap-1.5">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/[0.06]">
                    @forelse ($videos as $v)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            <td class="px-5 py-3 font-medium text-gray-900 dark:text-white max-w-[280px] truncate">{{ $v->titre }}</td>
                            <td class="px-5 py-3 text-gray-500">{{ $v->categorie?->nom ?? '—' }}</td>
                            <td class="px-5 py-3 text-gray-500">
                                @if ($v->duree_secondes)
                                    {{ gmdate($v->duree_secondes >= 3600 ? 'H:i:s' : 'i:s', $v->duree_secondes) }}
                                @else — @endif
                            </td>
                            <td class="px-5 py-3">
                                <x-status-badge :color="$v->est_publie ? 'green' : 'gray'" :label="$v->est_publie ? 'Publié' : 'Brouillon'" />
                            </td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.videos.edit', $v->id) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.videos.destroy', $v->id) }}"
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-10 text-center text-gray-400">Aucune vidéo trouvée.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
