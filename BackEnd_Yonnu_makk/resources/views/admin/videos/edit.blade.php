...
@section('content')
    <div class="mb-5">
        <a href="{{ route('admin.videos.index') }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-yoonu-600">
    ...
    <form method="POST" action="{{ route('admin.videos.update', $video->id) }}" enctype="multipart/form-data"
        ...
        @csrf @method('PUT')
        @include('admin.videos._form', ['submitLabel' => 'Enregistrer les modifications'])
    </form>
@endsection
