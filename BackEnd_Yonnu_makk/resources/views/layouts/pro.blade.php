...
<head>
    ...
    <title>@yield('title', 'Espace gynécologue') | {{ config('app.name', 'YOONU JIGEEN') }}</title>
...
<body class="bg-gray-50 dark:bg-[#1A0710] text-gray-900 dark:text-gray-100"
    ...
    @php $pro = auth('gynecologue_web')->user(); @endphp
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar"
            ...
            <div class="pt-7 pb-6 flex" :class="!$store.sidebar.isOpen ? 'xl:justify-center' : 'justify-start'">
                <a href="{{ route('pro.dashboard') }}" class="flex items-center gap-2.5">
            ...
            <nav class="flex flex-col overflow-y-auto flex-1 pb-4">
                <div>
                    <h2 class="mb-3 text-[0.65rem] uppercase tracking-[0.2em] flex text-gray-400 dark:text-gray-500"
                        ...
                        <span x-show="$store.sidebar.isOpen">Espace pro</span>
                        <span x-show="!$store.sidebar.isOpen">•••</span>
                    </h2>
                    <ul class="flex flex-col gap-1">
                        @php
                            $nav = [
                                ['pro.dashboard',        'Tableau de bord', 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
                                ['pro.patientes.*',      'Mes patientes', 'M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4zm6 0a3 3 0 10-2.83-4'],
                                ['pro.rendez-vous.*',    'Rendez-vous', 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
                                ['pro.notifications.*',  'Notifications', 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'],
                                ['pro.profil.*',         'Mon profil', 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'],
                            ];
                        @endphp
                        @foreach ($nav as [$route, $label, $icon])
                            <li>
                                <a href="{{ route(\Illuminate\Support\Str::replaceLast('.*', $route === 'pro.profil.*' ? '.edit' : '.index', $route)) }}"
            ...
            <header class="sticky top-0 w-full z-[9999] border-b border-gray-200 dark:border-white/[0.06] bg-white/80 dark:bg-[#1A0710]/80 backdrop-blur-xl">
                <div class="flex items-center justify-between px-4 py-3 xl:px-6 lg:py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-base xl:text-lg font-semibold dark:text-white font-display">@yield('title', 'Espace gynécologue')</h1>
                    </div>
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="flex items-center gap-2.5 text-sm">
                                <span class="hidden lg:block text-right leading-tight">
                                    <span class="block font-medium dark:text-white">Dr {{ $pro?->prenom }} {{ $pro?->nom }}</span>
                                ...
                                <span class="flex h-10 w-10 items-center justify-center rounded-full bg-yoonu-500 text-white font-bold text-sm">
                                    {{ strtoupper(mb_substr($pro?->prenom ?? 'G', 0, 1)) }}{{ strtoupper(mb_substr($pro?->nom ?? '', 0, 1)) }}
                                </span>
                            </button>
                            <div x-show="open" @click.away="open = false" x-transition x-cloak
                                ...
                                <div class="p-3 border-b border-gray-100 dark:border-white/[0.06]">
                                    <p class="px-2 text-sm font-semibold dark:text-white">Dr {{ $pro?->prenom }} {{ $pro?->nom }}</p>
                                    <p class="px-2 text-xs text-yoonu-600">Gynécologue</p>
                                </div>
                                <div class="p-3 space-y-1">
                                    <a href="{{ route('pro.profil.edit') }}" class="flex w-full items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.04]">
                                    ...
                                    <form method="POST" action="{{ route('pro.logout') }}">
                                        @csrf
