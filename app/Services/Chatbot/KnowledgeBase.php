...
class KnowledgeBase
{
    ...
    {
        return $this->searchFull($message, $langue)['response'] ?? null;
    }

    /**
     * Retourne la meilleure correspondance avec son ID, ou null.
     *
     * @return array{response: string, id: string}|null
     */
    public function searchFull(string $message, string $langue = 'fr'): ?array
    {
        $questions = $this->load();
        if (empty($questions)) {
            return null;
        ...
        return [
            'response' => $this->reponsePour($meilleur, $langue),
            'id'       => $meilleur['id'] ?? '',
        ];
    }
